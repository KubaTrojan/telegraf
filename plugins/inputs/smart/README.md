...
This plugin supports [nmve-cli](https://github.com/linux-nvme/nvme-cli) version 1.5 and above.
...
Association between a vid and company can be found [here](https://pcisig.com/membership/member-companies.).
...
*Worldwide Name* (WWN) or serial number to identify devices. On Linux block
...
```shell
  ## Optionally call smartctl and nvme-cli sequentially to gather disk attributes.
  ## By default smartctl and nvme are called in separate threads (goroutines) to gather disk attributes.
  ## Telegraf sometimes can show data from only one device. When that happen you should change the plugin configuration 
  ## to sequentially gather disk attributes instead of collecting it in separate threads (goroutines).
  # parallel_reads = true
```
